...
import { ShieldOff, ShieldCheck, Trash, Search } from "lucide-react";
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          await axios.patch(`${API_BASE_URL}/api/user/block/${id}`, {}, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          });
          ...
          <table className="table">
            ...
            <tbody className="table-body">
              {filteredUsers.length > 0 ? (
                  ...
                  <tr key={user._id} className="table-row">
                    ...
                    <td className="table-cell">
                      <div className="flex gap-x-1 w-14 h-14 overflow-hidden rounded-full border">
                        <img
                          src={user?.image ? user.image : getAvatar(index)}
                          alt={user.name}
                ...
                <tr className="table-row">
                  <td colSpan="6" className="table-cell text-center text-gray-500">No users found.</td>
                </tr>
              )}
            </tbody>
          </table>
